...
namespace usb::cdc {
  ...
  Error CDCDriver::SendSerial(const void* buf, int len) {
    ...
    if (auto err = ParentDevice()->BulkOut(ep_bulk_out_, buf_out, len)) {
      Log(kError, "%s:%d: BulkOut failed: %s\n", err.File(), err.Line(), err.Name());
      return err;
    }

    uint8_t* buf_in = new uint8_t[8];
    if (auto err = ParentDevice()->BulkIn(ep_bulk_in_, buf_in, 8)) {
      Log(kError, "%s:%d: BulkIn failed: %s\n", err.File(), err.Line(), err.Name());
      return err;
    }
    return MAKE_ERROR(Error::kSuccess);
  }
