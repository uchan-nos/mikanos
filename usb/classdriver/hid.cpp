...
namespace usb
{
  ...
  std::unique_ptr<Error> HIDBaseDriver::Initialize()
  {
    return MAKE_ERROR(Error::kNotImplemented);
  }

  std::unique_ptr<Error> HIDBaseDriver::SetEndpoint(const EndpointConfig &config)
  {
  ...
  std::unique_ptr<Error> HIDBaseDriver::OnEndpointsConfigured()
  {
  ...
  std::unique_ptr<Error> HIDBaseDriver::OnControlCompleted(EndpointID ep_id, SetupData setup_data,
  ...
  std::unique_ptr<Error> HIDBaseDriver::OnInterruptCompleted(EndpointID ep_id, const void *buf, int len)
  {
    if (ep_id.IsIn())
    {
      OnDataReceived();
