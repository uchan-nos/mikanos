...
namespace usb::xhci
{
  std::unique_ptr<Error> DeviceManager::Initialize(size_t max_slots)
  {
  ...
  */
  ...
  std::unique_ptr<Error> DeviceManager::AllocDevice(uint8_t slot_id, DoorbellRegister *dbreg)
  {
    if (slot_id > max_slots_)
    {
  ...
  std::unique_ptr<Error> DeviceManager::LoadDCBAA(uint8_t slot_id)
  {
    if (slot_id > max_slots_)
    {
      return MAKE_ERROR(Error::kInvalidSlotID);
  ...
  std::unique_ptr<Error> DeviceManager::Remove(uint8_t slot_id)
  {
